...
def norm_model(vertices):
    ...
    if norm_model is not None:
        sub_mean = norm_model.sub_mean_for_data_augmentation
    if sub_mean:     # TODO: check again
...
def setup_features_params(dataset_params, params):
    ...
    if 'dxdydz' in net_input:
        ...
        dataset_params.number_of_features += 3
    if 'vertices_indices' in net_input:
...
def mesh_data_to_walk_features_old(mesh_data, dataset_params):
